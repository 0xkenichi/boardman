/** BFF: recorded AFM fixture replay (GET /football/season/replay?matchday&home&away). */
...
export async function GET(req: Request) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch(`/api/stack/agentic/football/season/replay?${qs}`)
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/season/replay?${qs}`, { headers })
    const data = await res.json().catch(() => ({}))
    return NextResponse.json(data, { status: res.ok ? 200 : res.status })
  } catch {
    return NextResponse.json({ success: false, replay: null }, { status: 503 })
  }
}