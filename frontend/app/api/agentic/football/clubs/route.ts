/** BFF: agent-owned AFM clubs (GET list). */
...
export async function GET() {
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/clubs')
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/clubs`, { headers })
    const data = await res.json().catch(() => ({}))
    return NextResponse.json(data, { status: res.ok ? 200 : res.status })
  } catch {
    return NextResponse.json({ success: false, clubs: [] }, { status: 503 })
  }
}
