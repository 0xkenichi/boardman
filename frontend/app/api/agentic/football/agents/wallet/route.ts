/** BFF: wallet status for an AFM marketplace party (party_id query param). */
...
export async function GET(req: NextRequest) {
  ...
  const path = `/api/stack/agentic/football/agents/wallet?party_id=${encodeURIComponent(party)}`
  ...
  try {
    ...
    return NextResponse.json({ success: false, wallet: null }, { status: 503 })
  }
}
