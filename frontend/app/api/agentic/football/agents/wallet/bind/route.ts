/** BFF: bind a real (Circle) wallet to an AFM marketplace party. */
...
export async function POST(req: NextRequest) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/agents/wallet/bind', {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/agents/wallet/bind`, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
    })
