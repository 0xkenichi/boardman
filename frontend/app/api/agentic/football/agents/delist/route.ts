/** BFF: pull an AFM manager off the marketplace. */
...
export async function POST(req: NextRequest) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/agents/delist', {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/agents/delist`, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
    })
