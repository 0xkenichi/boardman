/** BFF: adopt a developer-built AFM manager from the marketplace. */
...
export async function POST(req: NextRequest) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/agents/acquire', {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/agents/acquire`, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
    })
