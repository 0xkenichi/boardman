/** BFF: make an offer on a listed AFM manager. */
...
export async function POST(req: NextRequest) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/agents/offer', {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/agents/offer`, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
    })
