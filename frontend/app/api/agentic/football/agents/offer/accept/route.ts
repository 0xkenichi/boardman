/** BFF: owner accepts a buyer's offer — the sale settles at the offered price. */
...
export async function POST(req: NextRequest) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/agents/offer/accept', {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/agents/offer/accept`, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
    })
