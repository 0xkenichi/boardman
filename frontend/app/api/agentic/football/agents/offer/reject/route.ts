/** BFF: owner rejects a buyer's offer — the listing stays live. */
...
export async function POST(req: NextRequest) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/agents/offer/reject', {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/agents/offer/reject`, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
    })
