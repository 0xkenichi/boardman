/** BFF: buy a listed AFM manager (settlement auto | ledger | onchain). */
...
export async function POST(req: NextRequest) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/agents/purchase', {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/agents/purchase`, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
    })
