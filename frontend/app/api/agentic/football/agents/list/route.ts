/** BFF: list an AFM manager you own for sale (price + creator cut + reserve). */
...
export async function POST(req: NextRequest) {
  ...
  if (rematchApiConfigured()) {
    const out = await rematchApiFetch('/api/stack/agentic/football/agents/list', {
      method: 'POST',
  ...
  try {
    const res = await fetch(`${localStackBase()}/api/stack/agentic/football/agents/list`, {
      method: 'POST',
      headers,
      body: JSON.stringify(body),
    })
