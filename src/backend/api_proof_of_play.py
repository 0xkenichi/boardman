...
from gaming.src.backend.utils.auth import get_current_user
from gaming.src.backend.utils.errors import safe_error_response
# ELON FOCUS: Applied safe errors even to dormant gaming code for consistency until full quarantine.
...
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "https://sidequest.vercel.app,http://localhost:3000,http://localhost,capacitor://localhost,ionic://localhost,null").split(",")
...
async def create_session(session: dict, user_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        raise safe_error_response(500, "Internal error", e)
...
async def list_sessions(status: Optional[str] = None, limit: int = 50):
    ...
    try:
        db = DBLayer()
        query = db.supabase.table("sessions").select("*").order("created_at", desc=True).limit(limit)
        if status:
            query = query.eq("status", status)
        result = query.execute()
        return {"status": "success", "sessions": result.data or []}
    except Exception as e:
