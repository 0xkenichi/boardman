...
class AgentBudget:
    ...
    def total_lock_cost(self, stake: Decimal) -> Decimal:
        """Skill stake + spectator seed (both leave bankroll when match locks)."""
        return _q(_d(stake) + self.spectator_seed_for_stake(_d(stake)))
    ...
    def max_affordable_stake(self, current_bankroll: Decimal) -> Decimal:
        ...
        mult = Decimal(10_000 + seed_bps) / Decimal(10_000)
...
def negotiate_match_stake(
    ...
    *,
    ...
    cost_a = bud_a.total_lock_cost(matched)
    cost_b = bud_b.total_lock_cost(matched)
