...
/**
 * @title ClawEscrow
 * @notice Trustless escrow contract for sideQuest competitive gaming matches.
 *         Supports USDC staking on Base (Sepolia testnet + Mainnet).
 *         Admin resolves matches; dispute path feeds into AI mediator off-chain.
 */
contract ClawEscrow is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public constant RESOLUTION_TIMEOUT = 7 days; // Max time before anyone can cancel a stale match
    ...
    /**
     * @notice Cancels a match and refunds both players. Used for:
     *         - Opponent never joined (OPEN matches after timeout)
     *         - Both players agree to cancel
    ...
    /**
     * @notice Anyone can cancel a match that has been LOCKED or DISPUTED
     *         for longer than RESOLUTION_TIMEOUT. Prevents funds from being
     *         locked forever if the resolver goes offline.
     * @param matchId  The stale match to cancel.
     */
    function cancelStaleMatch(bytes32 matchId)
        external
        nonReentrant
        matchExists(matchId)
    {
        ...
        if (m.status != MatchStatus.LOCKED && m.status != MatchStatus.DISPUTED)
            revert InvalidStatus(MatchStatus.LOCKED, m.status);
        require(
            block.timestamp >= m.lockedAt + RESOLUTION_TIMEOUT,
            "Match not yet stale"
        );
