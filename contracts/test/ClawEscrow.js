...
/**
 * ClawEscrow — V0 archive contract.
 * Functionally identical to BoardmanEscrow except FEE_BPS = 300 (3%).
 * Tests here confirm it still compiles and the 3% fee math is correct.
 */
describe("ClawEscrow (V0 archive)", function () {
  ...
  async function deploy() {
    ...
    const Escrow = await ethers.getContractFactory("ClawEscrow");
  ...
  it("initializes with 3% fee (V0) and correct state", async function () {
    ...
    expect(await escrow.FEE_BPS()).to.equal(300); // 3% — V0 legacy
  ...
  it("player1 creates a match and stakes USDC", async function () {
    const matchId = ethers.id("claw_match_1");
  ...
  it("reverts on zero stake", async function () {
    const matchId = ethers.id("claw_match_zero");
    await expect(
  ...
  it("reverts when stake exceeds max", async function () {
    const matchId = ethers.id("claw_match_max");
    await expect(
  ...
  it("reverts on duplicate matchId", async function () {
    const matchId = ethers.id("claw_match_dup");
  ...
  it("reverts when paused", async function () {
    const matchId = ethers.id("claw_match_paused");
  ...
  it("player2 joins and match locks", async function () {
    const matchId = ethers.id("claw_match_join");
    await escrow.connect(player1).createMatch(matchId, STAKE);
  ...
  it("player1 cannot join their own match", async function () {
    const matchId = ethers.id("claw_match_self");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await expect(
  ...
  it("reverts joining a non-existent match", async function () {
    const matchId = ethers.id("claw_match_nope");
    await expect(
  ...
  it("reverts joining an already locked match", async function () {
    const matchId = ethers.id("claw_match_locked");
    await escrow.connect(player1).createMatch(matchId, STAKE);
  ...
  // ─── Resolve Match (V0 = 3% fee) ─────────────────────────────────────

  it("resolver pays winner with 3% fee (V0)", async function () {
    const matchId = ethers.id("claw_match_resolve");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await escrow.connect(player2).joinMatch(matchId);

    const totalPot = STAKE * 2n;
    const expectedFee = (totalPot * 300n) / 10_000n; // 3%
  ...
  it("resolver can pay either player", async function () {
    const matchId = ethers.id("claw_match_resolve2");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await escrow.connect(player2).joinMatch(matchId);
  ...
  it("reverts resolver paying an address that is not a player", async function () {
    const matchId = ethers.id("claw_match_bad_winner");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await escrow.connect(player2).joinMatch(matchId);
    await expect(
  ...
  it("reverts resolving an OPEN match", async function () {
    const matchId = ethers.id("claw_match_open_resolve");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await expect(
  ...
  it("reverts non-resolver resolving", async function () {
    const matchId = ethers.id("claw_match_no_resolve");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await escrow.connect(player2).joinMatch(matchId);
    await expect(
  ...
  it("can resolve a DISPUTED match", async function () {
    const matchId = ethers.id("claw_match_dispute_resolve");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await escrow.connect(player2).joinMatch(matchId);
  ...
  it("resolver flags a match as disputed", async function () {
    const matchId = ethers.id("claw_match_dispute");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await escrow.connect(player2).joinMatch(matchId);
  ...
  it("cannot dispute an OPEN match", async function () {
    const matchId = ethers.id("claw_match_dispute_open");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await expect(
  ...
  it("cancel refunds both players", async function () {
    const matchId = ethers.id("claw_match_cancel");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await escrow.connect(player2).joinMatch(matchId);
  ...
  it("cancel OPEN match refunds only player1", async function () {
    const matchId = ethers.id("claw_match_cancel_open");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    const p1Before = await usdc.balanceOf(player1.address);
  ...
  it("cannot cancel a RESOLVED match", async function () {
    const matchId = ethers.id("claw_match_cancel_resolved");
    await escrow.connect(player1).createMatch(matchId, STAKE);
    await escrow.connect(player2).joinMatch(matchId);
  ...
  it("pause/unpause works", async function () {
    await escrow.connect(owner).pause();
    const matchId = ethers.id("claw_match_paused2");
    await expect(
      escrow.connect(player1).createMatch(matchId, STAKE)
    ).to.be.revertedWithCustomError(escrow, "EnforcedPause");
